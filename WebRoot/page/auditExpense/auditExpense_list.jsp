...
<body>
	<form method="post" action="<%=path %>/system/AuditExpenseServlet?m=list" id="listform">
...
<%
 ...
 %>				
				...
				<tr>
					...
					<c:if test="${not empty F020602 }">		
							 <a class="button border-red"
								href="<%=path%>/system/AuditExpenseServlet?m=del&id=<%=model.getId() %>" onclick="del()"><span
			...
			<div class="pagelist">
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/system/AuditExpenseServlet?m=list&currentPage=${pageBean.firstPage }&pageSize=${pageBean.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/system/AuditExpenseServlet?m=list&currentPage=${pageBean.prePage }&pageSize=${pageBean.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/system/AuditExpenseServlet?m=list&currentPage=${pageBean.nextPage }&pageSize=${pageBean.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/system/AuditExpenseServlet?m=list&currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
...
<script type="text/javascript">
  //1.改变每页显示记录数
  	function changePageSizer(){
  		...
  		var url = "${pageContext.request.contextPath}/system/AuditExpenseServlet?m=list&pageSize="+pageSize;
  	...
  	function changePager(){
  		...
  		var url="${pageContext.request.contextPath}/system/AuditExpenseServlet?m=list&currentPage="+curPage+"&pageSize=${pageBean.pageSize}";
  		window.location.href=url;
  	}
  	...
  	function editUser(id){
		$.post('/system/AuditExpenseServlet?m=get?'+id,function() {
			location.reload();
		});
	}
