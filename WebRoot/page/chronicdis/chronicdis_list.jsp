				...
				<td>${Status.index+1 }</td>
					...
					<c:if test="${not empty F010603 }">
							<a class="button border-main"
								href="<%=path%>/system/ChronicdisServlet?m=get&id=${emp.illcode }"><span
								class="icon-edit"></span> 修改</a>
					</c:if>		
					<c:if test="${not empty F010602 }">
							 <a class="button border-red"
								href="<%=path%>/system/ChronicdisServlet?m=del&id=${emp.illcode }"><span
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/system/ChronicdisServlet?m=list&currentPage=${pageBean.firstPage }&pageSize=${pageBean.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/system/ChronicdisServlet?m=list&currentPage=${pageBean.prePage }&pageSize=${pageBean.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/system/ChronicdisServlet?m=list&currentPage=${pageBean.nextPage }&pageSize=${pageBean.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/system/ChronicdisServlet?m=list&currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
