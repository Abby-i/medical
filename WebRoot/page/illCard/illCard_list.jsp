...
<%@page import="com.gxuwz.medical.dao.PayRecordDao"%>
<%@page import="com.gxuwz.medical.dao.AccountArchivesDao"%>
<%@page import="com.gxuwz.medical.dao.HomearchivesDao"%>
...
function openAddIllCard() {
	var index = layer.open({
		...
		maxmin : true,
		title: '新增慢病建档',
		content: '<%=path%>/system/IllCardServlet?m=input',
		end: function() {
			window.location.reload();
		}
	});
...
function openEditIllCard(id) {
	var index = layer.open({
		...
		maxmin : true,
		title: '修改慢病建档',
		content: '<%=path%>/system/IllCardServlet?m=get&id='+id,
		end: function() {
			window.location.reload();
		}
	});
...
<body>
		...
		<div class="panel admin-panel">
			...
			<form method="post" action="<%=path%>/system/IllCardServlet?m=list" id="listform">
			<div class="padding border-bottom">
				...
				<c:if test="${not empty F020401 }">		
					...
					<!-- <li>
					  	<input type="text" placeholder="请输入慢性病证号" name="illCard"
						class="input" style="width:150px; line-height:17px; display:inline-block"/> 
					</li> -->
					<!-- <li>
 ...
 %>				
  ...
  <%
 			int i = 0;
			while (iter.hasNext()) {
			IllCard model = (IllCard)iter.next();
  %>

				<tr>
					<td> <%=i+1 %> </td>
					<td><input type="checkbox" name="ids" value="<%=model.getId() %>" /></td>
					<td><%=archivesDao.quertById(model.getIdCard()).getName()%></td>
					...
					<td>
						...
						<c:if test="${not empty F020402 }">		
								...
								href="<%=path%>/system/IllCardServlet?m=del&id=<%=model.getId() %>" onclick="javascript:return del()"><span
			...
			<div class="pagelist">
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/system/IllCardServlet?m=list&currentPage=${pageBean.firstPage }&pageSize=${pageBean.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/system/IllCardServlet?m=list&currentPage=${pageBean.prePage }&pageSize=${pageBean.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/system/IllCardServlet?m=list&currentPage=${pageBean.nextPage }&pageSize=${pageBean.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/system/IllCardServlet?m=list&currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
...
<script type="text/javascript">
  //1.改变每页显示记录数
  	function changePageSizer(){
  		...
  		var url = "${pageContext.request.contextPath}/system/IllCardServlet?m=list&pageSize="+pageSize;
  	...
  	function changePager(){
  		...
  		var url="${pageContext.request.contextPath}/system/IllCardServlet?m=list&currentPage="+curPage+"&pageSize=${pageBean.pageSize}";
  		window.location.href=url;
  	}
  	...
  	function editUser(id){
		$.post('/system/IllCardServlet?m=get?'+id,function() {
			location.reload();
		});
	}
