<%@page import="com.gxuwz.medical.dao.AccountArchivesDao"%>
			...
			<div class="padding border-bottom">
				...
				<c:if test="${not empty F020101 }">		
					<li><a class="button border-main icon-plus-square-o"
						href="${pageContext.request.contextPath }/system/HomeArchivesServlet?m=input"> 添加家庭档案信息</a></li>
				</c:if>	
					<li>搜索：</li>
...
<%
 ...
 <%
 			int i = 0;
			while (iter.hasNext()) {
			Homearchives home = (Homearchives)iter.next();
  %>
				<tr>
				<td><%=i+1 %> </td>
					...
					<td><%=archivesDao.queryPropertyCount(home.getHomeid()) %></td>
					...
					<c:if test="${not empty F020104 }">	
						...
						<c:if test="${not empty F020201 }">		
							<a class="button border-red "
								href="<%=path%>/system/HomeArchivesServlet?m=addhomeRole&id=<%=home.getHomeid() %>&household=<%=home.getHousehold() %>"><span
								class="icon-plus-square-o"></span> 添加成员</a>
						</c:if>
						<c:if test="${not empty F020103 }">			
							<a class="button border-main"
								href="<%=path%>/system/HomeArchivesServlet?m=get&id=<%=home.getHomeid() %>"><span
								class="icon-edit"></span> 修改</a>
						</c:if>
						<c:if test="${not empty F020102 }">		
							 <a class="button border-red"
								href="<%=path%>/system/HomeArchivesServlet?m=del&id=<%=home.getHomeid() %>" onclick="return del()"><span
			...
			<div class="pagelist">
	    			...
	    			<c:otherwise>
	    					<a href="${pageContext.request.contextPath }/system/HomeArchivesServlet?m=list&currentPage=${pageBean.firstPage }&pageSize=${pageBean.pageSize}">首页</a> 
							<a href="${pageContext.request.contextPath }/system/HomeArchivesServlet?m=list&currentPage=${pageBean.prePage }&pageSize=${pageBean.pageSize}">上一页</a> 
	    			</c:otherwise>
	    			</c:choose>
	    			...
	    			<c:choose>
						...
						<c:otherwise>
							<a href="${pageContext.request.contextPath }/system/HomeArchivesServlet?m=list&currentPage=${pageBean.nextPage }&pageSize=${pageBean.pageSize}">下一页</a>
							<a href="${pageContext.request.contextPath }/system/HomeArchivesServlet?m=list&currentPage=${pageBean.totalPage }&pageSize=${pageBean.pageSize}">尾页</a>
							</c:otherwise>   		
    				</c:choose>
...
<script type="text/javascript">
  //1.改变每页显示记录数
  	function changePageSizer(){
  		...
  		var url = "${pageContext.request.contextPath}/system/HomeArchivesServlet?m=list&pageSize="+pageSize;
  	...
  	function changePager(){
  		...
  		var url="${pageContext.request.contextPath}/system/HomeArchivesServlet?m=list&currentPage="+curPage+"&pageSize=${pageBean.pageSize}";
  		window.location.href=url;
  	}
  	...
  	function editUser(menuid){
		$.post('/system/HomeArchivesServlet?m=get?'+menuid,function() {
			location.reload();
		});
	}
...
　　　　　　 title: '查看家庭成员',　　
 ...
 　　　　　　　　//location.reload();　　  // 重新加载当前页面
 				var url = "${pageContext.request.contextPath}/system/HomeArchivesServlet?m=list";
 				window.location.href=url;
 　　　　}
 　　});
  }
