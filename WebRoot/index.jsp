...
<body style="background-color:#f2f9fd;">
	...
	<div class="leftnav">
		...
		<ul style="display:block">
			...
			<li><a href="<%=path %>/system/FarmerServlet?m=list" target="right"><span
					class="icon-caret-right"></span>农合机构管理</a></li>
			</c:if>
			<c:if test="${not empty F0106 }">
			<li><a href="<%=path %>/system/ChronicdisServlet?m=list" target="right"><span
			...
			<li><a href="<%=path %>/system/PayStantardServlet?m=list" target="right"><span
					class="icon-caret-right"></span>缴费标准设置</a></li>
			</c:if>
		</ul>
		</c:if>
		...
		<ul>
			...
			<li><a href="<%=path %>/system/HomeArchivesServlet?m=list" target="right"><span
			...
			<li><a href="<%=path %>/system/IllCardServlet?m=list" target="right"><span
					class="icon-caret-right"></span>慢病证管理</a></li>
			</c:if>
			<c:if test="${not empty F0205 }">
			<li><a href="<%=path %>/system/IllExpenseServlet?m=search" target="right"><span
			...
			<c:if test="${not empty F0206 }">
			<li><a href="<%=path %>/system/RemittanceExpenseServlet?m=list" target="right"><span
					class="icon-caret-right"></span>慢病报销汇款审核</a></li>
			</c:if>
		</ul>
		</c:if>
