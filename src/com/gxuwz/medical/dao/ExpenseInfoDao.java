...
public class ExpenseInfoDao extends GenericDao<IllExpense>{
	...
	public int queryCount(String illCode,String town, String village, String group, String name) {
		StringBuffer sb=new StringBuffer("select count(*) from t_ill_expense where 1=1 ");
		if(illCode!=null && !illCode.trim().equals("")){
			sb.append(" and illCode = '"+illCode+"'");
		}
	...
	public List<IllExpense> queryExpenseInfo(String illCode, String town, String village, String group, String name,
		 ...
		 if(illCode!=null && !illCode.trim().equals("") && !illCode.trim().equals("1")){
				sb.append(" and illCode = '"+illCode+"'");
		 }
		 if(town!=null && !town.trim().equals("")){
				sb.append(" and nongheCard like '"+town+"%'");
		 ...
		 sb.append("and auditStatus =2 ");
		 sb.append("order by idCard asc limit ?,?");
		 try {
			 conn = DbUtil.getConnection();
			 ptmt = conn.prepareStatement(sb.toString());
